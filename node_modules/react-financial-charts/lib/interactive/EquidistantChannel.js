...
export class EquidistantChannel extends React.Component {
    constructor(props) {
        super(props);
        this.handleDragChannel = (index, newXYValue) => {
            this.setState({
                override: Object.assign({ index }, newXYValue),
            });
        };
        this.handleDragChannelComplete = (moreProps) => {
        ...
        this.handleDrawChannel = (xyValue) => {
            const { current } = this.state;
    ...
    render() {
        ...
        const tempChannel = isDefined(current) && isDefined(current.endXY)
            ? React.createElement(EachEquidistantChannel, Object.assign({ interactive: false }, current, { appearance: appearance, hoverText: hoverText }))
            : null;
