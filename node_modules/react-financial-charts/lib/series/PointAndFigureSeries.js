...
export class PointAndFigureSeries extends React.Component {
    constructor() {
        ...
        this.renderSVG = (moreProps) => {
            ...
            const { stroke = PointAndFigureSeries.defaultProps.stroke, fill = PointAndFigureSeries.defaultProps.fill, strokeWidth, className, } = this.props;
            const columns = getColumns(xScale, xAccessor, yScale, plotData);
            return (React.createElement("g", { className: className }, columns.map((col, idx) => (React.createElement("g", { key: idx, className: col.className, transform: `translate(${col.offset[0]}, ${col.offset[1]})` }, col.boxes.map((box, i) => {
                if (col.direction > 0) {
                    return (React.createElement("g", { key: `${idx}-${i}` },
                        React.createElement("line", { className: "up", strokeWidth: strokeWidth, stroke: stroke.up, fill: fill.up, x1: 0, y1: box.open, x2: box.columnWidth, y2: box.close }),
                        React.createElement("line", { className: "up", strokeWidth: strokeWidth, stroke: stroke.up, fill: fill.up, x1: 0, y1: box.close, x2: box.columnWidth, y2: box.open })));
                }
                return (React.createElement("ellipse", { key: `${idx}-${i}`, className: "down", strokeWidth: strokeWidth, stroke: stroke.down, fill: fill.down, cx: box.columnWidth / 2, cy: (box.open + box.close) / 2, rx: box.columnWidth / 2, ry: box.boxHeight / 2 }));
            }))))));
        };
