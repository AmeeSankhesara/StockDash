...
class HoverTooltip extends React.Component {
    constructor() {
        ...
        this.renderSVG = (moreProps) => {
            ...
            return (React.createElement("g", null,
                React.createElement("rect", { x: centerX - pointWidth / 2, y: 0, width: pointWidth, height: height, fill: bgFill, opacity: bgOpacity }),
                React.createElement("g", { className: "react-financial-charts-tooltip-content", transform: `translate(${x}, ${y})` },
    ...
    render() {
        return React.createElement(GenericComponent, { svgDraw: this.renderSVG, canvasDraw: this.drawOnCanvas, drawOn: ["mousemove", "pan"] });
    }
}
...
function defaultBackgroundShapeSVG({ fill, stroke, opacity }, { height, width }) {
    return React.createElement("rect", { height: height, width: width, fill: fill, opacity: opacity, stroke: stroke });
}
function defaultTooltipSVG({ fontFamily, fontSize, fontFill }, content) {
    ...
    for (let i = 0; i < content.y.length; i++) {
        ...
        tspans.push(React.createElement("tspan", { key: `L-${i}`, x: X, y: textY, fill: y.stroke }, y.label));
        tspans.push(React.createElement("tspan", { key: i }, ": "));
        tspans.push(React.createElement("tspan", { key: `V-${i}` }, y.value));
    }
    return React.createElement("text", { fontFamily: fontFamily, fontSize: fontSize, fill: fontFill },
        React.createElement("tspan", { x: X, y: startY }, content.x),
        tspans);
}
